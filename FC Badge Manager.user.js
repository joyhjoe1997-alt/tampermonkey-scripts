...
(function () {
    ...
    // Cross-extension compatibility (Tampermonkey, Greasemonkey 4+, Violentmonkey)
    const gmSet = (typeof GM_setValue === 'function') ? GM_setValue : (typeof GM !== 'undefined' && GM.setValue) ? (k, v) => GM.setValue(k, v) : (k, v) => localStorage.setItem('fcbm_' + k, JSON.stringify(v));
    const gmGet = (typeof GM_getValue === 'function') ? GM_getValue : (typeof GM !== 'undefined' && GM.getValue) ? (k, d) => GM.getValue(k, d) : (k, d) => { try { const v = localStorage.getItem('fcbm_' + k); return v !== null ? JSON.parse(v) : d; } catch { return d; } };
    ...
    if (isFCResearch) {
        ...
        function trySaveBadge() {
            ...
            if (result && result.id) {
                const prev = gmGet('BADGE_ID', '');
                gmSet('BADGE_ID',    result.id);
                gmSet('BADGE_LOGIN', result.login || '');
                gmSet('BADGE_WHID',  result.whid || '');
                gmSet('BADGE_TIME',  Date.now());
    ...
    if (isBarcodeGen) {
        const bgBadge = gmGet('BADGE_ID',    '');
        const bgLogin = gmGet('BADGE_LOGIN', '');
        const bgWhid  = gmGet('BADGE_WHID',  '');
        const bgTime  = gmGet('BADGE_TIME',  0);
